...
// Código usado pelo algoritmo FIFO
typedef struct {
    int *f;
    int inicio;
    int fim;
} struct_fila;
// A fila armazena as páginas que estão na memória (o 1º da fila corresponde a página que está a mais tempo na memória)
struct_fila fila;
//-----------------------------------
...
int fifo(int8_t** page_table, int num_pages, int prev_page,
    ...
    // Remove da fila a página que está na memória a mais tempo
    extern struct_fila fila;
    int page = fila.f[fila.inicio];
    fila.inicio = ++fila.inicio % num_frames;
    return page;
}
...
int simulate(int8_t **page_table, int num_pages, int *prev_page, int *fifo_frm,
    ...
    // Código usado pelo algoritmo FIFO, adiciona na fila a página requisitada
    extern struct_fila fila;
    fila.f[fila.fim] = virt_addr;
    fila.fim = ++fila.fim % num_frames;
    // ------------------------------------
...
int main(int argc, char **argv) {
    ...
    // Código usado pelo algoritmo FIFO
    extern struct_fila fila;
    fila.f = (int*) malloc (num_frames * sizeof(int));
    fila.inicio = 0;
    fila.fim = 0;
    // ---------------------------------
